"""ML evaluation for the clip and gp_standard ablation arms (F1 three-way
comparison), using the exact same CV procedure as run_ml_da_gp_wgan.py."""
...
for arm in ['clip', 'gp_standard']:
    pts = np.load(f'../../outputs/gan_pts_{arm}.npy')
    feats = np.load(f'../../outputs/gan_feats_{arm}.npy')
    print(f'--- {arm}: {len(pts)} points ---')
    res, roc, pr = run_cv(pts, feats, return_curves=True)
    for m, v in res.items():
        print(f"  {m}: AUC={v['AUC']:.4f}  TSS={v['TSS']:.4f}")
    np.save(f'../../outputs/cv_{arm}.npy', {'res': res, 'roc': roc, 'pr': pr}, allow_pickle=True)
    print(f'Saved cv_{arm}.npy')
