...
*Bands A–E read in order, band C right to left: A supplies the admissible level
...
covariates into a handful of ordinal suitability levels before modeling —
an analytic hierarchy process, frequency-ratio or information-value step that
encodes expert judgment into the data. Every presence record is therefore
confined to a finite lattice of permitted values.
...
guarantee, and no experiment here separates the two. A second artifact — a
local-geometry fingerprint left by the averaging inherent to interpolation —
is characterised rather than solved. See §7.1 of the paper.
...
| Tables 6, 7 — downstream AUC and TSS | `evaluation/evaluate_arms.py` |
...
| Figs. 12, 13 — AUC/TSS surfaces, bootstrap intervals | `figures/generate_batch3.py` |
| Fig. 14 — local-geometry fingerprint | `evaluation/construction_audit.py` |
| Figs. 15, 16 — the four evaluation conditions | `evaluation/e4_dual_artifact.py`, `figures/generate_e4.py` |
...
100–125 s each. Arms carrying the per-interpolant manifold check take
20–23 min, because each interpolated sample needs a 7-NN query against the
...
Not included here. The Alberta wildfire records (3,370 points, 1984–2024,
